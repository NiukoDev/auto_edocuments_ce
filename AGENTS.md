...
- Validaciones documentales implementadas: `PD` busca `tipodocumento=PED_no=2`; `PS` busca `tipodocumento=PED_no=1`; `XMLPD` busca `tipodocumento=XMLC`. Todas escanean recursivamente nombres/rutas contra llave compacta o pedimento y marcan `a`/`x`. El `ESTATUS` general sigue en `PENDIENTE`.
